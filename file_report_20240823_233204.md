...
def resize_video(input_path, output_path, width=None, height=None, interpolation=cv2.INTER_LINEAR, codec='mp4v', use_gpu=False):
    """
    Resize a video while maintaining its aspect ratio, with optional GPU acceleration.
    ...
    :param codec: Codec used for the output video.
    :param use_gpu: Boolean flag to use GPU (CUDA) if available.
    """
    cap = cv2.VideoCapture(input_path)
    if not cap.isOpened():
        raise ValueError("Could not open the video file.")

    # Determine the codec for the output video
    ...
    gpu_enabled = use_gpu and cv2.cuda.getCudaEnabledDeviceCount() > 0
    if gpu_enabled:
        gpu_cap = cv2.cuda_GpuMat()
    ...
    while True:
        ...
        if gpu_enabled:
            ...
            gpu_cap.upload(frame)
            resized_frame = cv2.cuda.resize(gpu_cap, (width, height), interpolation=interpolation)
